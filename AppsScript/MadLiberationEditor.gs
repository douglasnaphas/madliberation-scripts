...
function exportScript() {
  ...
  for (var i = 0; i < body.getNumChildren(); i++) {
    ...
    var formatType = child.getAttributes()[DocumentApp.Attribute.HEADING];
    Logger.log("text format type: " + formatType);
    if (formatType == "Title" || formatType == "Heading 1") {
      mlText = mlText + "# ";
    }
    if (formatType == "Subtitle") mlText = mlText + "#### ";
    if (formatType == "Heading 2") mlText = mlText + "## ";
    if (formatType == "Heading 3") mlText = mlText + "### ";
    if (formatType == "Heading 4") mlText = mlText + "#### ";
    if (formatType == "Heading 5") mlText = mlText + "##### ";
    if (formatType == "Heading 6") mlText = mlText + "###### ";
    var elText = child.asText().getText();
    // Logger.log(elText);
    if (elText) mlText = mlText + elText + "\n\n";
    var containsPageBreak =
  ...
  // Logger.log(exportText);
  // The HTTP request could be to a public Mad Liberation service that turns an ML Markdown file
  // into ML JSON. Alternatively, MLJSAPI could call (or implement) such a service for use after
  // upload.
  var fetchData = { text: exportText };
  var fetchOpts = {
    method: "post",
  ...
  var response = UrlFetchApp.fetch(
    "https://zf9bc2c6fl.execute-api.us-east-1.amazonaws.com/Prod/",
    fetchOpts
  );
